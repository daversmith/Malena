...
namespace ml
{
    ...
    /**
     * @brief Manages mutual exclusion across a set of @c RadioButton instances.
     * @ingroup GraphicsControls
     *
     * @c RadioGroup holds non-owning references to @c RadioButton objects.
     * When any button in the group is clicked, @c RadioGroup deselects all
     * others, selects the clicked one, and fires @c onSelectionChanged.
     *
     * @c RadioGroup is not a @c Component — it does not render or register
     * with @c CoreManager. Add each @c RadioButton individually via
     * @c addComponent; let @c RadioGroup handle selection logic only.
     *
     * ### Usage
     * @code
     * ml::RadioGroup group;
     ...
     * hard.setLabel("Hard");
     *
     * easy.setPosition({200.f, 100.f});
     * medium.setPosition({200.f, 130.f});
     * hard.setPosition({200.f, 160.f});
     *
     * group.add(easy);
     * group.add(medium);
     * group.add(hard);
     * group.selectFirst();
     *
     * group.onSelectionChanged([](ml::RadioButton& btn){
     *     std::cout << "Selected: " << btn.getLabel() << "\n";
     * });
     *
     * addComponent(easy);
     * addComponent(medium);
     ...
     * @endcode
     *
     * @see RadioButton, Selectable
     */
    class RadioGroup
    {
    public:
        /**
         * @brief Add a @c RadioButton to this group.
         *
         * Wires the button's @c onClick to the group's selection logic.
         * The button must outlive this @c RadioGroup.
         *
        ...
        /**
         * @brief Programmatically select a button by zero-based index.
         *
         * Deselects all others, selects the one at @p index, and fires
         * @c onSelectionChanged. Has no effect if @p index is out of range
        ...
        /**
         * @brief Select the first button in the group.
         *
         * Convenience wrapper for @c select(0). Safe to call on an empty group.
         */
        void selectFirst();

        /** @brief Return a pointer to the currently selected button, or @c nullptr. */
        [[nodiscard]] RadioButton* getSelected() const;

        /**
         * @brief Return the zero-based index of the selected button.
         *
         * Returns @c std::string::npos if no button is selected.
         */
        [[nodiscard]] std::size_t getSelectedIndex() const;

        /** @brief Return the number of buttons in this group. */
        [[nodiscard]] std::size_t size() const;

        /**
         * @brief Register a callback invoked whenever the selection changes.
         *
         * @param callback Function invoked with a reference to the newly
         *                 selected @c RadioButton.
         */
        void onSelectionChanged(std::function<void(RadioButton&)> callback);

    private:
        std::vector<RadioButton*>         _buttons;
        RadioButton*                      _selected = nullptr;
        std::function<void(RadioButton&)> _onSelectionChanged;
